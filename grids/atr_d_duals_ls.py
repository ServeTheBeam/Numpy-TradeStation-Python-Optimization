"""Grid definitions for ATR-D DualSymbol v1.5 Long/Short.

Same 16-param layout as LS, but with DualS-specific ranges.
The DualS-LS kernel takes 6 data arrays.

Parameter order MUST match the kernel's p0..p15 signature:
...
PARAM_ORDER = [
    ...
    "equity_price_method",
    "short_multiplier", "entry_mode", "trend_filter_period",
    "trend_veto_threshold", "both_triggered_mode",
]

INT_PARAMS = {"atr_period", "bb_length", "scaling_method",
              "atr_buy_period", "atr_smooth_period", "equity_price_method",
              "entry_mode", "trend_filter_period", "both_triggered_mode"}
...
# ---------------------------------------------------------------------------
# ts_coarse — Match TS coarse grid (from _ATR-D-DualS-v1.5-LS_coarse.csv)
# ---------------------------------------------------------------------------
_register("ts_coarse", {
    "atr_period":           np.array([8, 32, 56], dtype=np.int64),
    "atr_multiplier":       np.arange(3, 19, 3, dtype=np.float64),         # 3,6,9,12,15,18
    ...
    "atr_buy_multiplier":   np.array([0.5, 1.5, 2.5], dtype=np.float64),
    ...
    "entry_mode":           np.array([0, 2], dtype=np.int64),
    "trend_filter_period":  np.array([4, 16], dtype=np.int64),
    "trend_veto_threshold": np.array([0.0, 0.05], dtype=np.float64),
    "both_triggered_mode":  np.array([0, 1], dtype=np.int64),
})
...
_register("phase1_slim", {
    ...
    "atr_buy_multiplier":   np.arange(0.5, 3.1, 0.5, dtype=np.float64),
    ...
    "entry_mode":           np.array([0, 1, 2], dtype=np.int64),
    "trend_filter_period":  np.array([4, 10, 16], dtype=np.int64),
    "trend_veto_threshold": np.array([0.0, 0.02, 0.05, 0.10], dtype=np.float64),
    "both_triggered_mode":  np.array([0, 1], dtype=np.int64),
})

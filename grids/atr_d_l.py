"""Grid definitions for ATR-D v1.5 Long-only.
...
# Canonical parameter order — must match kernel and chunk runner
PARAM_ORDER = [
...
def _register(name, grid_dict):
    """Register a grid, ensuring correct parameter order."""
    ordered = {}
...
# ---------------------------------------------------------------------------
# ts_coarse — Match TS coarse grid exactly (77,760 combos)
# ---------------------------------------------------------------------------
_register("ts_coarse", {
...
# ---------------------------------------------------------------------------
# phase1_slim — 58M combos, ~4 min with 32 threads. Primary WFO grid.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# phase1 — 116M combos, includes equity_price_method=[0,1]
# ---------------------------------------------------------------------------
_register("phase1", {
    "atr_period":          np.array([4, 16, 28, 40, 52, 64], dtype=np.int64),
    "atr_multiplier":      np.arange(2, 25, 2, dtype=np.float64),
...
def get_grid(name: str) -> dict[str, np.ndarray]:
    """Return a named grid (ordered dict of param_name -> array)."""
...
def count_combinations(grid: dict[str, np.ndarray]) -> int:
    """Total parameter combinations in a grid."""
    total = 1
...
def decode_flat_indices(flat_indices, param_names, param_arrays):
    """Reconstruct parameter values from flat indices.

    Uses the same last-dimension-fastest ordering as run_chunk_wfo.
    """
